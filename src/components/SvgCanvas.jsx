  ...
  function layoutToSvg(layout) {
    ...
    if (showFurniture) {
      try {
        ...
        svg += `<g class="furniture">${furnitureItemsToSvg(items)}</g>`;
      } catch (e) {
      ...
      <div ref={wrapperRef} style={containerStyle}>
        <div style={{transform:`scale(${scale})`, transformOrigin:'top left', display:'inline-block'}}>
          <div dangerouslySetInnerHTML={{
            __html: layout ? layoutToSvg(layout) : "<div style='padding:12px;color:#444'>Generate Layout</div>"
          }} />
        </div>
      </div>
    </div>
